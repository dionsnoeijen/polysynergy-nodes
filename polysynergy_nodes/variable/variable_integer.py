...
@node(name="Variable Integer", category="variable", icon='integer.svg', version=1.0)
class VariableInteger(Node):
    value: int | str = NodeVariableSettings(label="Value", dock=True, has_in=True)
    values: dict[str, str] = NodeVariableSettings(label="Values", dock=True, has_in=True, default={})

    true_path: bool | int = PathSettings(label="Result", info="The integer value with placeholders replaced")
    ...
    async def execute(self):
        try:
            ...
            # Convert to integer
            # Handle various formats: "123", "123.0", 123, 123.5 (will truncate)
            if isinstance(value_replaced, str):
                value_replaced = value_replaced.strip()
                if value_replaced == "":
                    result = 0
                else:
                    # Try to convert - handles "123" and "123.0"
                    result = int(float(value_replaced))
            elif isinstance(value_replaced, (int, float)):
                result = int(value_replaced)
            else:
                raise ValueError(f"Cannot convert {type(value_replaced).__name__} to integer")
